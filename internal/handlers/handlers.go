...
func (m *Repository) PostShowLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		m.App.Session.Put(r.Context(), "error", "Invalid login crediencial")
...
// AdminShowReservation shows the resetvation in the admin tool
func (m *Repository) AdminShowReservation(w http.ResponseWriter, r *http.Request) {
	// /admin/reservations/new/9
	exploded := strings.Split(r.RequestURI, "/")
...
func (m *Repository) AdminPostReservationCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, room := range rooms {
		...
		// that dones not exist in our posted data, and if the restriction id > 0, then it is a block
		// we need to remove.

...
func (m *mysqlDBRepo) GetRestrictionsForRoomByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	...
	query := `
		select id, ifnull(reservation_id, 0), restriction_id, room_id, start_date, end_date
...
func (m *mysqlDBRepo) InsertBlockForRoom(id int, startDate time.Time) error {
	...
	`
	_, err := m.DB.ExecContext(ctx, query, startDate, startDate.AddDate(0, 0, 1), id, time.Now(), time.Now())
	if err != nil {
		log.Println(err)
		return err
	}
